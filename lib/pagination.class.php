<?php
  ...
  *
  ...
  class pagination
  {
     ...
     * @access public  
     */
    public function __construct($array, $curPage = null, $perPage = null, $pageurl)
    {
      $this->array   = $array;
      ...
      if (($this->pages) > 1) {
        ...
        if ($this->page != 1) {
          if ($this->_showFirstAndLast) {
            $plinks[] = ' <a href="../1/'.$queryUrl.'">&laquo;&laquo; First </a> ';
          }
          $plinks[] = ' <a href="../'.$this->pageurl.'/'.($this->page - 1).'">&laquo; Prev</a> ';
        }
        
        // Assign all the page numbers & links to the array
        for ($j = 1; $j < ($this->pages + 1); $j++) {
          if ($this->page == $j) {
            $links[] = ' <a class="selected"><strong>'.$j.'</strong></a> '; // If we are on the same page as the current item
          } else {
            $links[] = ' <a href="../'.$this->pageurl.'/'.$j.'">'.$j.'</a> '; // add the link to the array
          }
        }

        // Assign the 'next page' if we are not on the last page
        if ($this->page < $this->pages) {
          $slinks[] = ' <a href="'.($this->page + 1).'" class="next-page"> Next &raquo; </a> ';
          if ($this->_showFirstAndLast) {
            $slinks[] = ' <a href="'.($this->pages).$queryUrl.'"> Last &raquo;&raquo; </a> ';
          }
        }
